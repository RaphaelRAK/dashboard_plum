...
import { Avatar, Typography, Space, Spin, List, Select, Input, Tag, message, Button, Empty, Badge, Tooltip, Divider, Row, Col, notification } from 'antd';
...
const ClaimComponent: React.FC<ClaimComponentProps> = ({ claims: initialClaims, messages: initialMessages }) => {
    ...
    const cloudflare_url_storage_images = import.meta.env.VITE_CLOUDFLARE_STORAGE_URL_FOR_IMAGES || '';
    const adminId = import.meta.env.VITE_CURRENT_USER_ID;
    const [messagesState, setMessagesState] = useState<MessageChat[]>(initialMessages);
    const [lastMessagesMap, setLastMessagesMap] = useState<LastMessagesMap>({});
    const channelRef = useRef<RealtimeChannel | null>(null);
    const [isLoadingMessages, setIsLoadingMessages] = useState(true);
    const dispatch = useDispatch();
    const claimsFromRedux = useSelector((state: RootState) => state.claims.claims);
    ...
    const updateLastMessages = async () => {
        ...
        try {
            ...
            await Promise.all(claims.map(async (claim) => {
                ...
                try {
                    const channelMessages = await fetchMessagesByChannel(claim.channel_id.toString());
            ...
            console.error("Erreur lors de la mise à jour des derniers messages:", error);
        } finally {
    ...
    useEffect(() => {
        const loadAllMessages = async () => {
            ...
            try {
                const messages = await fetchMessagesByChannel(selectedClaim.channel_id.toString());
        ...
        if (selectedClaim) {
            ...
            if (selectedClaim.channel_id) {
                const channel = supabaseClient
                    .channel(`message_chat_${selectedClaim.channel_id}`)
                    .on('postgres_changes', {
                        event: '*',
                        schema: 'public',
                        table: 'message_chat',
                        ...
                        switch (payload.eventType) {
                            case 'INSERT':
                                setMessagesState(prevMessages => [...prevMessages, payload.new as MessageChat]);
                                updateLastMessages();
                                ...
                                break;
                            case 'UPDATE':
                                setMessagesState(prevMessages => prevMessages.map(msg => msg.id === payload.new.id ? payload.new : msg));
                                updateLastMessages();
                                break;
                            case 'DELETE':
                                setMessagesState(prevMessages => prevMessages.filter(msg => msg.id !== payload.old.id));
                                updateLastMessages();
                                break;
                        }
                    })
    ...
    const handleStatusChange = async (claimId: string, newStatus: string) => {
        try {
            ...
            if (selectedClaim && (selectedClaim.id === actualClaimId || selectedClaim.claim_id === actualClaimId || selectedClaim.claim_id?.toString() === actualClaimId)) {
                const updatedSelectedClaim = {
                    ...selectedClaim,
                    status: newStatus
                };
                setSelectedClaim(updatedSelectedClaim);
                console.log('✅ Réclamation sélectionnée mise à jour:', updatedSelectedClaim);
            }
            ...
            const updatedClaimsArray = claims.map(claim => {
                if (
                    ...
                    console.log('✅ Réclamation trouvée dans la liste et mise à jour:', claim.claim_id || claim.id);
            ...
            try {
                ...
                console.error('❌ Erreur lors de la mise à jour du store Redux:', error);
            }
            ...
            api.error({
                message: 'Erreur',
                description: 'Impossible de mettre à jour le statut de la réclamation dans la base de données',
                placement: 'topRight',
                duration: 4
            });
        }
        ...
        // console.log('😍😍😍😍😍😍😍😍😍😍😍😍😍😍');
            const matchesSearch = searchText
                ? claim.order_id.toString().includes(searchText) ||
                  (claim.public_profile?.first_name || '').toLowerCase().includes(searchText.toLowerCase()) ||
                  (claim.public_profile?.last_name || '').toLowerCase().includes(searchText.toLowerCase()) ||
    ...
    useEffect(() => {
        if (selectedClaim?.public_profile?.avatar) {
            console.log(`Chemin de l'avatar selectedClaim: ${
    ...
    return (
        <div className={`claims-chat-container ${isMobile && isChatVisible ? 'mobile-chat-visible' : ''}`} style={containerStyle}>
            {contextHolder}
            <div className="claims-list" style={{ backgroundColor: mode === 'dark' ? '#1f1f1f' : '#fff' }}>
                <div className="claims-search-filter" style={{ 
                    ...
                    <Title level={4} style={{ color: mode === 'dark' ? '#e8eaed' : '#202124' }}>Réclamations</Title>
                    ...
                    <Divider style={{ margin: '0.5rem 0 1rem', borderColor: mode === 'dark' ? '#303030' : '#e0e0e0' }} />
                </div>
                ...
                {isLoadingMessages ? (
                    ...
                    <Empty 
                        description={<Text style={{ color: mode === 'dark' ? '#e8eaed' : '#202124' }}>Aucune réclamation trouvée</Text>}
                    ...
                    <div className="claims-list-container">
                        <List
                            ...
                            renderItem={claim => {
                                ...
                                const isSelected = selectedClaim && selectedClaim.id === claim.id;
                                const claimKey = claim.channel_id || claim.id;
                                const lastMessageInfo = lastMessagesMap[claimKey] || { message: "Aucun message", time: "" };
                                    ...
                                    <div 
                                        ...
                                        style={{ 
                                            backgroundColor: mode === 'dark' 
                                                ? (isSelected ? 'rgba(26, 115, 232, 0.15)' : '#242424') 
                                                : (isSelected ? 'rgba(26, 115, 232, 0.08)' : '#fff'),
                                    ...
                                    >
                                        <div style={{ display: 'flex', width: '100%' }}>
                                            <div className="claim-avatar">
                                                ...
                                                >
                                                    <Avatar 
                                                        src={customerInfo.avatar && cloudflare_url_storage_images 
                                            ...
                                            <div className="claim-content">
                                                <div className="claim-header">
                                                    <span className="claim-name" style={{ color: mode === 'dark' ? '#e8eaed' : '#202124' }}>
                                                        {customerInfo.first_name} {customerInfo.last_name}
                                                    </span>
                                                    
                                                    <div className="claim-meta">
                                                        <span className="claim-time" style={{ color: mode === 'dark' ? '#9aa0a6' : '#5f6368' }}>
                                                            {lastMessageInfo.time || ''}
                                                        </span>
                                                        
                                                        <Tooltip title={getStatusTranslation(claim.status)}>
                                                            <Tag color={getStatusColor(claim.status)} style={{ marginLeft: '0.5rem' }}>
                                                                {getStatusIcon(claim.status)} {getStatusTranslation(claim.status)}
                                                            </Tag>
                                                        </Tooltip>
                                                    </div>
                                                </div>
                                                ...
                                                {claim.order_id && (
                                                    <Text type="secondary" style={{ fontSize: '0.75rem', marginTop: '0.25rem', color: mode === 'dark' ? '#9aa0a6' : '#5f6368' }}>
            ...
            <div className="chat-container" style={{ backgroundColor: mode === 'dark' ? '#1f1f1f' : '#fff' }}>
                {selectedClaim ? (
                    <>
                        <div className="chat-header" style={{ backgroundColor: mode === 'dark' ? '#242424' : '#fff', borderColor: mode === 'dark' ? '#303030' : '#e0e0e0' }}>
                            {isMobile && (
                                <Button 
                                    type="text" 
                            ...
                            <div className="chat-header-info">
                                <Avatar 
                                    src={selectedClaim?.public_profile?.avatar && cloudflare_url_storage_images 
                                        ? `${cloudflare_url_storage_images}${selectedClaim.public_profile.avatar.startsWith('/') ? '' : '/'}${selectedClaim.public_profile.avatar}` 
                                        : null}
                                    icon={!selectedClaim?.public_profile?.avatar && <UserOutlined />}
                                    size={isMobile ? 36 : 40}
                                    style={{ 
                                        marginRight: '1rem',
                                ...
                                />
                                <div>
                                    <Title level={5} style={{ margin: 0, color: mode === 'dark' ? '#e8eaed' : '#202124' }}>
                                        {selectedClaim.public_profile?.first_name} {selectedClaim.public_profile?.last_name}
                                    </Title>
                                    
                                    <Space align="center" size={4} wrap={isMobile} style={{ fontSize: isMobile ? '0.75rem' : 'inherit' }}>
                                        <Text type="secondary" style={{ color: mode === 'dark' ? '#9aa0a6' : '#5f6368' }}>
                                            #{selectedClaim.id}
                                        </Text>
                                        
                                        {!isMobile && selectedClaim.order_id && (
                                            <>
                                                <Divider type="vertical" style={{ borderColor: mode === 'dark' ? '#303030' : '#e0e0e0' }} />
                                                <Text type="secondary" style={{ color: mode === 'dark' ? '#9aa0a6' : '#5f6368' }}>
                            ...
                            <Space>
                                <Select
                                    ...
                                    onChange={(value) => handleStatusChange(
                                        selectedClaim.claim_id ? selectedClaim.claim_id.toString() : selectedClaim.id,
                                        value
                                    )}
                                    style={{ width: isMobile ? '120px' : '180px' }}
                                    dropdownStyle={{ backgroundColor: mode === 'dark' ? '#242424' : '#fff' }}
                                    size={isMobile ? "small" : "middle"}
                                >
                                    <Option value="OPEN_UNPROCESSED">
                                        <ExclamationCircleOutlined style={{ marginRight: '8px', color: '#ea4335' }} />
                                        Non traité
                                    </Option>
                                    <Option value="OPEN_IN_PROGRESS">
                                        <ClockCircleOutlined style={{ marginRight: '8px', color: '#1a73e8' }} />
                                        En cours
                                    </Option>
                                    <Option value="PENDING_RESPONSE">
                                        <CommentOutlined style={{ marginRight: '8px', color: '#fbbc04' }} />
                                        En attente
                                    </Option>
                                    <Option value="RESOLVED">
                                        <CheckCircleOutlined style={{ marginRight: '8px', color: '#34a853' }} />
                                        Résolu
                                    </Option>
                                </Select>
                        ...
                        {selectedClaim.channel_id ? (
                            ...
                            <div className="empty-state">
                                <ExclamationCircleOutlined className="empty-state-icon" style={{ color: mode === 'dark' ? '#9aa0a6' : '#5f6368' }} />
                                <Title level={4} style={{ color: mode === 'dark' ? '#e8eaed' : '#202124' }}>
                    ...
                    <div className="empty-state">
                        <MessageOutlined className="empty-state-icon" style={{ color: mode === 'dark' ? '#9aa0a6' : '#5f6368' }} />
                        <Title level={4} style={{ color: mode === 'dark' ? '#e8eaed' : '#202124' }}>
